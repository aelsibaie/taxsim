...
def make_graph(main_income_type, file_name, filing_status, child_dep, income_ratios, payroll, step, top_range=10000):
    ...
    # Save CSVs
    current_law_df.to_csv(taxsim.RESULTS_DIR + file_name + '-current_law_graph_data.csv')
    ...
    ax.plot(current_law_df["gross_income"], current_law_df[rate_type], drawstyle='steps-pre', label='Current Law')
    ax.plot(house_2018_df["gross_income"], house_2018_df[rate_type], drawstyle='steps-pre', label='House 2018 Proposal')
    ax.plot(senate_2018_df["gross_income"], senate_2018_df[rate_type], drawstyle='steps-pre', label='Senate 2018 Proposal')
